...
class SAC_Trainer:
    def __init__(
        self, replay_buffer, state_space, action_space, hidden_dim, action_range
    ):
        self.replay_buffer = replay_buffer

        self.soft_q_net1 = QNetworkLSTM(state_space, action_space, hidden_dim).to(
            device
        )
        self.soft_q_net2 = QNetworkLSTM(state_space, action_space, hidden_dim).to(
            device
        )
        self.target_soft_q_net1 = QNetworkLSTM(
            state_space, action_space, hidden_dim
        ).to(device)
        self.target_soft_q_net2 = QNetworkLSTM(
            state_space, action_space, hidden_dim
        ).to(device)
        self.policy_net = SAC_PolicyNetworkLSTM(
            state_space, action_space, hidden_dim, action_range
        ).to(device)
    ...
    def update(
        ...
        target_q_min = (
            torch.min(predict_target_q1, predict_target_q2) - self.alpha * next_log_prob
        )
        target_q_value = (
...
# choose env
ENV = ["Reacher", "Pendulum-v1", "HalfCheetah-v2"][1]
if ENV == "Reacher":
    NUM_JOINTS = 2
    LINK_LENGTH = [200, 140]
    ...
    action_space = spaces.Box(
        low=-1.0, high=1.0, shape=(env.num_actions,), dtype=np.float32
    )
