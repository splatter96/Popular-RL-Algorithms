...
class UniformReplayBuffer:
    ...
    def sample(self):
        ...
        return (
            torch.tensor(self.s[ind]).to(self.device),
...
class MultiAgentUniformReplayBuffer(UniformReplayBuffer):
    ...
    def __init__(
        ...
        if state_type == "image":
            self.s = np.zeros(
                (self.max_size, self.N_agents, *state_shape), dtype=np.float32
            )
            self.s2 = np.zeros(
                (self.max_size, self.N_agents, *state_shape), dtype=np.float32
            )
...
class UniformReplayBuffer_BootDQN(UniformReplayBuffer):
    ...
    def __init__(
        self, state_type, state_shape, buffer_length, batch_size, device, K, mask_p
    ):
        super().__init__(
            state_type=state_type,
            state_shape=state_shape,
...
class UniformReplayBuffer_LSTM(UniformReplayBuffer):
    ...
    def sample(self) -> tuple:
        ...
        # sample indices
        bat_indices = np.random.randint(
            low=self.history_length, high=self.size, size=self.batch_size
        )
        ...
        s_hist = np.zeros(
            (self.batch_size, self.history_length, self.state_shape), dtype=np.float32
        )
        ...
        if self.disc_actions:
            ...
            a_hist = np.zeros(
                (self.batch_size, self.history_length, self.action_dim),
                dtype=np.float32,
            )
        ...
        for i, b_idx in enumerate(bat_indices):
            ...
            for j in range(1, self.history_length + 1):
                if self.d[b_idx - j] == True:
                    ...
                    s_hist[i] = np.roll(
                        s_hist[i], shift=-(self.history_length - j + 1), axis=0
                    )
                    a_hist[i] = np.roll(
                        a_hist[i], shift=-(self.history_length - j + 1), axis=0
                    )
                    break
        ...
        if self.disc_actions:
            a2_hist = np.zeros(
                (self.batch_size, self.history_length, 1), dtype=np.int64
            )
        else:
            a2_hist = np.zeros(
                (self.batch_size, self.history_length, self.action_dim),
        ...
        for i, b_idx in enumerate(bat_indices):
            # take data
            s2_hist[i, :, :] = self.s[
                (b_idx - self.history_length + 1) : (b_idx + 1), :
            ]
            a2_hist[i, :, :] = self.a[
                (b_idx - self.history_length + 1) : (b_idx + 1), :
            ]
            ...
            for j in range(1, self.history_length):
                if self.d[b_idx - j] == True:
                    ...
                    # move non-zero experiences to the beginning
                    s2_hist[i] = np.roll(
                        s2_hist[i], shift=-(self.history_length - j), axis=0
                    )
                    a2_hist[i] = np.roll(
                        a2_hist[i], shift=-(self.history_length - j), axis=0
                    )
                    break
...
class UniformReplayBufferEnvs_BootDQN(UniformReplayBuffer_BootDQN):
    ...
    def __init__(
        ...
        super().__init__(
            state_type, state_shape, buffer_length, batch_size, device, K, mask_p
        )
        self.envs = [None] * buffer_length
